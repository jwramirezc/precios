...
class Module {
  constructor(data) {
    ...
    this.url =
      data.url ||
      GENERAL_CONFIG?.links?.defaultModuleUrl ||
      'https://www.saiasoftware.com/';
...
class PricingCalculator {
  ...
  calculateExtraUserCost(includedUsers) {
    ...
    return (
      this.calculateUserCostForN(this.userCount) -
      this.calculateUserCostForN(includedUsers)
    );
  }
  ...
  calculateQuantityModulesCost() {
    return this.modules
      ...
      .reduce((total, m) => {
        return (
          total +
          this._getBlockPrice(m.quantity_config.pricing_key, m.selectedQty)
        );
      }, 0);
  }
  ...
  calculateModulesCost() {
    ...
    return selectedCalculableModules.reduce((total, module) => {
      ...
      if (
        module.pricing_tier &&
        this.pricingTiers[module.pricing_tier] !== undefined
      ) {
        price = this.pricingTiers[module.pricing_tier];
      } else {
        console.warn(
          `Price not found for tier: ${module.pricing_tier} in module ${module.name}`
        );
      }
      return total + price;
    }, 0);
  ...
  calculateStorageCost() {
    ...
    if (remainingGB > 0 && sortedTiers.length > 0) {
      totalStorageCost +=
        remainingGB * sortedTiers[sortedTiers.length - 1].pricePerGB;
    }
  ...
  calculatePresetBreakdown() {
    ...
    const extraModules = this.modules.filter(
      m =>
        ...
        m.calculable &&
        m.visible &&
        !includedModuleIds.includes(m.id)
    );
    ...
    const extraStorageCost = this.calculateExtraStorageCost(
  ...
  calculateBreakdown() {
    ...
    const subtotal =
      platformFee + userCost + modulesCost + storageCost + quantityCost;
